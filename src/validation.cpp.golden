...
// Timecoin: emission Fibonacci
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    if (nHeight == 0) return 0;
    const CAmount INITIAL_SUBSIDY = 377 * COIN;
    const double PHI = 1.6180339887498948482;
    const int BLOCKS_PER_MONTH = 4380;
    auto fibMonth = [](int m) -> int {
        if (m <= 0) return 0;
        if (m == 1) return 1;
        int a = 1, b = 1;
        for (int i = 3; i <= m; ++i) { int c = a + b; a = b; b = c; }
        return b;
    };
    int season = 1;
    int blockStart = 1;
